...
std::ofstream fstability("my_stability.txt");
int score_map[8][8] =
{
    ...
    int set_stability(Point action)
    {
        ...
        for(auto d: directions)
        {
            ...
            while(p.x>=0 && p.x<=SIZE &&p.y>=0 && p.y<=SIZE)
            {
                p.x += d.x;
                p.y += d.y;
                if(get_disc(p) == get_next_player(cur_player) || get_disc(p)==0) //遇到敵人了  或空的
                {
                    fstability<<"meet different:"<<p.x<<","<<p.y<<endl;
                    meet_different = true;
                    break;
                }
            }
        ...
        fstability<<"stability = "<<stability<<"-----------------\n\n";
